...
class Lexer {
    ...
    private Pop(): void // calls into reader.pop, but also updates current position and linetable
    {
...
class Node {
    ...
    static AreSame(n1: Node, n2: Node): boolean {
        if (n1.kind != n2.kind)
            return false;
        if (n1.name != n2.name)
            return false;
        if (n1.hasParens != n2.hasParens)
            return false;
        // Check children
        if (n1.children.length != n2.children.length)
            return false;
        for (var i = 0; i < n1.children.length; i++) {
            if (!this.AreSame(n1.children[i], n2.children[i]))
                return false;
        }
...
class Parser {
    ...
    ParseFuncCallArgs(): Node {
        ...
        while (this.tok != token.tok_rparen) {
...
function needsParens(node: Node): boolean {
    if (node.parentNode) {
        if (node.level > node.parentNode.level) // larger level is lower precedence
        {
            // special case for divide as it shows everything above the line
            // this is disabled as it is actually nice to see parentheses that are 
            // required for correctness in the original expression
...
function nodeToTex(node: Node): string {
    ...
    switch (node.kind) {
        ...
        case AstKind.AST_MULT:
            if (Node.AreSame(node.children[0], node.children[1]))
                out += nodeToTexSquare(node.children[0]);
            else
        ...
        case AstKind.AST_FUNCTIONCALL:
            {
                ...
                switch (call) {
                    ...
                    case "\\mathit{hypotl}":
                        out += "\\sqrt{";
                        out += nodeToTexSquare(args.children[0]);
                        out += " + ";
                        out += nodeToTexSquare(args.children[1]);
                        out += "}";
                        break;
                    case "\\mathit{sin}":
...
function nodeToTexWithParens(node: Node): string {
    var out: string = nodeToTex(node);
    switch (node.kind) {
        case AstKind.AST_CONSTANT:
        case AstKind.AST_CONSTANTREAL:
        case AstKind.AST_BOOLLITERAL:
...
function nodeToTexSquare(node: Node): string {
    var out: string = "{" + nodeToTexWithParens(node) + "}^2";
    return out;
}
